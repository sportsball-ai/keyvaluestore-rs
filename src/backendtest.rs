...
macro_rules! test_backend {
    ($f:expr) => {
        ...
        async fn test_z_range_by_score() {
            ...
            // -Inf
            let members = b
                .z_range_by_score("foo", f64::NEG_INFINITY, 1.0, 0)
                .await
                .unwrap();
            ...
            // +Inf
            let members = b
                .z_range_by_score("foo", -0.5, f64::INFINITY, 0)
                .await
                .unwrap();
            ...
            // Rev
            {
                ...
                // -Inf
                let members = b
                    .z_rev_range_by_score("foo", f64::NEG_INFINITY, 1.0, 0)
                    .await
                    .unwrap();
                ...
                // +Inf
                let members = b
                    .z_rev_range_by_score("foo", -0.5, f64::INFINITY, 0)
                    .await
                    .unwrap();
            ...
            // Update
            {
                ...
                let members = b
                    .z_range_by_score("update-test", 1.5, 2.5, 0)
                    .await
                    .unwrap();
                ...
                let members = b
                    .z_range_by_score("update-test", 2.5, 3.5, 0)
                    .await
                    .unwrap();
                assert_eq!(vec!["foo"], members);
            }
        ...
        async fn test_z_count() {
            ...
            assert_eq!(
                b.z_count("foo", f64::NEG_INFINITY, f64::INFINITY)
                    .await
                    .unwrap(),
                6
            );
            ...
            for i in 0..1100 {
                b.z_add(
                    "big",
                    [i.to_string().as_bytes().to_vec(), big_value.clone()].concat(),
                    0.0,
                )
                .await
                .unwrap();
